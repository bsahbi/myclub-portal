...
export const OrganizerProfileView: React.FC<OrganizerProfileViewProps> = ({ organizerId }) => {
  const { organizers, competitions, language, t,} = useApp();
  ...
  const orgCompetitions = competitions.filter((c) =>
    organizer.upcomingEvents.includes(c.id) || c.organizer.name.toLowerCase().includes(organizer.name[language].toLowerCase().slice(0, 10))
  );
