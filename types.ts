      ...
      <div className="relative w-full max-w-2xl my-8 bg-white dark:bg-slate-900 rounded-3xl shadow-2xl border border-slate-200 dark:border-slate-800 overflow-hidden">
        <div className="p-6 bg-gradient-to-r from-red-600 to-rose-700 text-white relative">
          ...
          <p className="text-white/90 text-sm mt-1">
            {language === 'ar'
              ? 'سجّل الآن مجاناً وأصبح ضمن دليل فضاءاتنا الرياضية الموثوق.
              أختر نوع العضوية التي تريد التسجيل بها:'
              : 'Sign up for free and join our verified sports directory.
            Choose the membership type you want to register as:'}
          </p>
        </div>
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-5">
              ...
              <button
                ...
                className={`flex flex-col items-center gap-1.5 py-3 px-2 rounded-xl border-2 transition-all cursor-pointer text-center ${
                  memberType === value
                    ...
                    : 'border-transparent bg-slate-50 dark:bg-slate-800/60 text-slate-600 dark:text-slate-400 hover:border-slate-300 dark:hover:border-slate-600'
                }`}
              >
          ...
          <div>
            ...
            <input
              ...
              placeholder={memberType === 'coach' ? 'e.g. Coach Mehdi Al-Alawi' : memberType === 'athlete' ? 'e.g. Bilal "The Cobra" Kasmi' : memberType === 'organizer' ? 'e.g. Arab Combat Federation' : 'e.g. Casablanca MMA Academy'}
            />
          </div>
            ...
            <div className="grid grid-cols-1 gap-4">
              <div>
                ...
                <input
                  ...
                  className="w-full px-3.5 py-2.5 rounded-xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 text-slate-900 dark:text-white text-sm focus:ring-2 focus:ring-red-600 focus:outline-none"
                  placeholder={language === 'ar' ? 'مثال: حزام أسود BJJ +3 년' : 'e.g. BJJ Black Belt +3 years'}
                />
                <p className="text-[10px] text-slate-500 dark:text-slate-400 mt-1">
                  {language === 'ar' ? 'اللقب أو الرتبة أو الشهادة التي تمنحك المصداقية.' : 'Your title, rank, or certification that gives you credibility.'}
                </p>
              </div>
              <div>
                <label className="block text-xs font-semibold text-slate-700 dark:text-slate-300 mb-1">
                  {t('subscribe.skillsLabel')}
                </label>
                <input
                  type="text"
                  ...
                  className="w-full px-3.5 py-2.5 rounded-xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 text-slate-900 dark:text-white text-sm focus:ring-2 focus:ring-red-600 focus:outline-none"
                  placeholder="Private coaching, sparring sessions, Gi/Nogi classes, fight preparation..."
                />
                <p className="text-[10px] text-slate-500 dark:text-slate-400 mt-1">
                  {language === 'ar' ? 'م 진입장벽이 낮은 3~5 개 스킬 쉼표 구분 키워드.' : '3-5 keyword skills separated by commas.'}
                </p>
              </div>
            </div>
          )}
          ...
          {memberType === 'athlete' && (
            <div className="grid grid-cols-1 gap-4">
              <div>
                <label className="block text-xs font-semibold text-slate-700 dark:text-slate-300 mb-1">
                  {t('subscribe.categoryLabel')}
                </label>
                <input
                  type="text"
                  value={formData.categoryOrJurisdiction}
              ...
              <div>
                ...
                <input
                  ...
                  className="w-full px-3.5 py-2.5 rounded-xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 text-slate-900 dark:text-white text-sm focus:ring-2 focus:ring-red-600 focus:outline-none"
                  placeholder={language === 'ar' ? '예: 브랜드 후원에 관심 있습니다 + 노출 계획.' : 'e.g. Open to brand sponsorships + exposure plan.'}
                />
              </div>
            </div>
          )}
            ...
            <div className="grid grid-cols-1 gap-4">
              ...
              <div>
                ...
                <input
                  ...
                  className="w-full px-3.5 py-2.5 rounded-xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 text-slate-900 dark:text-white text-sm focus:ring-2 focus:ring-red-600 focus:outline-none"
                  placeholder={language === 'ar' ? '예: https://arabcombat.com 또는 @arab_combat' : 'e.g. https://arabcombat.com or @arab_combat'}
                />
              </div>
            </div>
          )}
            ...
            <div className="grid grid-cols-1 gap-4">
              <div>
                ...
                <input
                  ...
                  onChange={(e) => setFormData({ ...formData, titleOrRank: e.target.value })}
                  className="w-full px-3.5 py-2.5 rounded-xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 text-slate-900 dark:text-white text-sm focus:ring-2 focus:ring-red-600 focus:outline-none"
                  placeholder={language === 'ar' ? '예: IBJJF 공인 아카데미 + 주 6 일 개장.' : 'e.g. IBJJF registered academy + 6 days/week.'}
                />
              </div>
              <div>
                <label className="block text-xs font-semibold text-slate-700 dark:text-slate-300 mb-1">
                ...
                <textarea
                  ...
                  placeholder={language === 'ar' ? '클럽 소개, 시설, 코칭 철학, 차별점 등.' : 'Brief club description, facilities, coaching philosophy, what sets you apart.'}
                />
              </div>
            </div>
          )}
          ...
          <div>
            ...
            <textarea
              ...
              onChange={(e) => setFormData({ ...formData, bio: e.target.value })}
              className="w-full px-3.5 py-2.5 rounded-xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 text-slate-900 dark:text-white text-sm focus:ring-2 focus:ring-red-600 focus:outline-none resize-none"
              placeholder={language === 'ar' ? '자기소개 또는 단체에 대한 한 줄 소개.' : 'Short bio or one-line about you or your organization.'}
            />
          </div>

          {/* Website/social for club */}
          {memberType === 'club' && (
            <div>
              <label className="block text-xs font-semibold text-slate-700 dark:text-slate-300 mb-1">
                {t('subscribe.websiteLabel')} *
          ...
          {/* Pricing/sponsorship (shown for some types) */}
          {(memberType === 'club' || memberType === 'coach') && (
            <div>
              <label className="block text-xs font-semibold text-slate-700 dark:text-slate-300 mb-1">
                {t('subscribe.pricingLabel')}
              </label>
              <input
                type="text"
                value={formData.pricingOrSponsorship}
                onChange={(e) => setFormData({ ...formData, pricingOrSponsorship: e.target.value })}
                className="w-full px-3.5 py-2.5 rounded-xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 text-slate-900 dark:text-white text-sm focus:ring-2 focus:ring-red-600 focus:outline-none"
                placeholder={language === 'ar' ? '월 회비 또는 후원 강요 등.' : 'Monthly fee or sponsorship pitch etc.'}
              />
            </div>
          )}
